...
#####################################################################
TASplot <-
function(data,color="blue",size=3,shape=1) {
 dataTAS <- adjRock (data)
...
function(dataTAS) {
 ...
 TAS = data.frame(S = as.numeric(dataTAS$SiO2), NK = as.numeric(dataTAS$K2O) + as.numeric(dataTAS$Na2O)) #define TAS again
